...
int main() {
    ...
    cout<<torch::cuda::is_available?"cuda yes":"cuda no";
    auto tensor_img = torch::from_blob(new_image.data,{1,new_image.rows,new_image.cols,3}, torch::kByte).to(device);
    ...
    for(int i=0;i<scores.size(0);i++)
    {
        ...
        if(score>0.5)
        {
            int x1 = boxCoord[i][0].item<int>();
            int y1 = boxCoord[i][1].item<int>();
            int x2 = boxCoord[i][2].item<int>();
            int y2 = boxCoord[i][3].item<int>();
            int label = boxIndex[i].item<int>();
